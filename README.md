...
* The packages that have to be manually installed
    * The latest `pycocotools` repository link: https://github.com/philferriere/cocoapi
        ...
        * Need to install Visual C++ Build Tools to build [(download link)](https://go.microsoft.com/fwlink/?LinkId=691126)
        * Read `/PythonAPI/Makefile` to get the command prompt to build
        * Open cmd(***as administrater***) and move the directory to `\PythonAPI`, then install the `Cython` and `numpy` with the command `pip3 install Cython` and `pip3 install numpy`, then build the cocoapi with the command `python setup.py build_ext --inplace`
